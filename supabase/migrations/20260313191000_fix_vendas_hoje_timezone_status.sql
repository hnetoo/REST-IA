-- Migration: Fix Vendas Hoje - Timezone and Status Update
-- Description: Update v_vendas_hoje with timezone precision and all status variations
-- Created: 2026-03-13 19:10:00

-- 1. Atualizar função get_dashboard_metrics para fix do "Vendas Hoje"
...
BEGIN
  ...
  v_vendas_total := (SELECT COALESCE(SUM(total_amount), 0) FROM public.orders WHERE status IN ('closed', 'paid', 'completed', 'finalized', 'FECHADO'));
  
  -- CÁLCULOS TEMPORAIS (SELECTS SIMPLES)
  -- Vendas do período selecionado
  v_vendas_periodo := (SELECT COALESCE(SUM(total_amount), 0) FROM public.orders WHERE created_at::date >= v_start_date AND created_at::date <= v_end_date AND status IN ('closed', 'paid', 'completed', 'finalized', 'FECHADO'));
  v_orders_count := (SELECT COUNT(*) FROM public.orders WHERE created_at::date >= v_start_date AND created_at::date <= v_end_date AND status IN ('closed', 'paid', 'completed', 'finalized', 'FECHADO'));
  
  -- Vendas de hoje (TIMEZONE PRECISO E TODOS OS STATUS) - FIX CRÍTICO
  v_vendas_hoje := (SELECT COALESCE(SUM(total_amount), 0) 
    ...
    WHERE (created_at AT TIME ZONE 'UTC' AT TIME ZONE 'Africa/Luanda')::date = (now() AT TIME ZONE 'Africa/Luanda')::date
      AND status IN ('closed', 'paid', 'completed', 'finalized', 'FECHADO')
  );
  ...
  -- GERAR CHART DATA CONFORME PERÍODO (ESTRUTURA PARA GRÁFICOS)
  IF p_period = 'HOJE' THEN
    -- HOJE: Agrupar por hora do dia (24 pontos - 0h às 23h)
    v_chart_data := (
      SELECT json_agg(
        json_build_object(
      ...
      FROM (
        SELECT 
          hora,
          COALESCE(SUM(total_amount), 0) as hourly_total,
          COUNT(*) as hourly_orders
        FROM (
          SELECT 
            date_trunc('hour', created_at AT TIME ZONE 'UTC' AT TIME ZONE 'Africa/Luanda') as hora, 
            total_amount
          FROM public.orders
          WHERE (created_at AT TIME ZONE 'UTC' AT TIME ZONE 'Africa/Luanda')::date = (now() AT TIME ZONE 'Africa/Luanda')::date
            AND status IN ('closed', 'paid', 'completed', 'finalized', 'FECHADO')
  ...
  ELSIF p_period = 'SEMANA' THEN
    ...
    v_chart_data := (
      ...
      FROM (
        ...
        FROM (
          SELECT 
            (created_at AT TIME ZONE 'UTC' AT TIME ZONE 'Africa/Luanda')::date as dia, 
            total_amount
          FROM public.orders
          WHERE (created_at AT TIME ZONE 'UTC' AT TIME ZONE 'Africa/Luanda')::date >= v_start_date 
            AND (created_at AT TIME ZONE 'UTC' AT TIME ZONE 'Africa/Luanda')::date <= v_end_date
            AND status IN ('closed', 'paid', 'completed', 'finalized', 'FECHADO')
        ) as daily_data
        GROUP BY dia
        ORDER BY dia
  ...
  ELSIF p_period = 'ANO' THEN
    ...
    v_chart_data := (
      ...
      FROM (
        ...
        FROM (
          SELECT 
            date_trunc('month', created_at AT TIME ZONE 'UTC' AT TIME ZONE 'Africa/Luanda')::date as mes, 
            total_amount
          FROM public.orders
          WHERE (created_at AT TIME ZONE 'UTC' AT TIME ZONE 'Africa/Luanda')::date >= v_start_date 
            AND (created_at AT TIME ZONE 'UTC' AT TIME ZONE 'Africa/Luanda')::date <= v_end_date
