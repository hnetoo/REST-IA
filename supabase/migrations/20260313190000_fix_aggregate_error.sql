-- Migration: Fix Aggregate Error 42803 - Simple Variables
-- Description: Fix nested aggregate functions by using simple SELECT assignments
-- Created: 2026-03-13 19:00:00

-- 1. Correção da função SQL com variáveis separadas
...
BEGIN
  ...
  v_vendas_total := (SELECT COALESCE(SUM(total_amount), 0) FROM public.orders WHERE status IN ('closed', 'paid', 'completed'));
  
  -- CÁLCULOS TEMPORAIS (SELECTS SIMPLES)
  -- Vendas do período selecionado
  v_vendas_periodo := (SELECT COALESCE(SUM(total_amount), 0) FROM public.orders WHERE created_at::date >= v_start_date AND created_at::date <= v_end_date AND status IN ('closed', 'paid', 'completed'));
  v_orders_count := (SELECT COUNT(*) FROM public.orders WHERE created_at::date >= v_start_date AND created_at::date <= v_end_date AND status IN ('closed', 'paid', 'completed'));
  
  -- Vendas de hoje (apenas do dia atual em Angola) - VARIÁVEL SEPARADA
  v_vendas_hoje := (SELECT COALESCE(SUM(total_amount), 0) FROM public.orders WHERE created_at::date = CAST(timezone('Africa/Luanda', now()) AS DATE) AND status IN ('closed', 'paid', 'completed'));
  ...
  -- GERAR CHART DATA CONFORME PERÍODO (SEM AGREGAÇÃO ANINHADA)
