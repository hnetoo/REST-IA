...
-- ============================================
-- Verificar se a funcao ja existe (schema_real.sql ja a tem)
-- Funcao: cleanup_old_active_orders()
...
-- ============================================
-- VERIFICACAO: Execute estas queries para confirmar
-- ============================================

-- Verificar se as funcoes foram criadas:
-- SELECT proname FROM pg_proc WHERE proname IN ('cleanup_old_active_orders', 'archive_old_audit_logs', 'get_database_size_stats');

-- Verificar tamanho atual das tabelas:
-- SELECT * FROM get_database_size_stats();

-- Testar limpeza de active_orders (preview):
-- SELECT COUNT(*) as would_delete FROM active_orders WHERE updated_at < NOW() - INTERVAL '24 hours' AND status != 'ABERTO';

-- Testar arquivamento de audit_logs (preview):
-- SELECT COUNT(*) as would_archive FROM audit_logs WHERE timestamp < NOW() - INTERVAL '90 days';
