    ...
    <div className="p-8 h-full bg-background flex flex-col overflow-hidden animate-in fade-in duration-700">
      ...
      >
            ...
            >
              ...
              {isDesignMode && (
                <div className="absolute -bottom-12 flex gap-2 animate-in zoom-in">
                   <button onClick={() => handleTableMove(table.id, 'left')} className="p-2 bg-slate-800 rounded-lg text-white hover:bg-orange-500" aria-label="Mover mesa para esquerda"><ChevronLeft size={12}/></button>
                   <div className="flex flex-col gap-1">
                      <button onClick={() => handleTableMove(table.id, 'up')} className="p-2 bg-slate-800 rounded-lg text-white hover:bg-orange-500" aria-label="Mover mesa para cima"><ChevronUp size={12}/></button>
                      <button onClick={() => handleTableMove(table.id, 'down')} className="p-2 bg-slate-800 rounded-lg text-white hover:bg-orange-500" aria-label="Mover mesa para baixo"><ChevronDown size={12}/></button>
                   </div>
                   <button onClick={() => handleTableMove(table.id, 'right')} className="p-2 bg-slate-800 rounded-lg text-white hover:bg-orange-500" aria-label="Mover mesa para direita"><ChevronRight size={12}/></button>
                </div>
              )}
            </div>
          );
        ...
        {!isDesignMode && (
          <button 
            onClick={handleAddTable}
            className="w-28 h-28 rounded-[1.5rem] border-2 border-dashed border-white/5 hover:border-white/20 hover:bg-white/5 transition-all flex flex-col items-center justify-center text-slate-600 hover:text-white group absolute right-10 bottom-10"
          >
