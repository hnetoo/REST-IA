  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 p-6 overflow-y-auto pb-24">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        {/* Card Verde - Impostos Acumulados (7% Regime Simplificado - TEMPO REAL) */}
        <div className="bg-gradient-to-br from-green-600 to-green-700 rounded-xl p-6 text-white relative group">
          <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
            <div className="bg-black/90 text-white text-xs rounded px-2 py-1 max-w-xs">
              <p className="font-semibold mb-1">💰 Impostos (7%)</p>
              <p className="text-xs">Calcular 7% sobre Faturação Hoje</p>
              <p className="text-xs font-bold mt-1">7% sobre {formatKz(todayRevenue)} = {formatKz(todayRevenue * 0.07)}</p>
            </div>
          </div>
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold">Impostos (7%)</h3>
            <DollarSign className="h-8 w-8 text-green-200" />
          </div>
          <div className="space-y-2">
            <p className="text-3xl font-bold text-green-100">
              {formatKz(todayRevenue * 0.07)}
              {todayRevenue > 0 && (
                <span className="ml-2 text-xs bg-green-500 px-2 py-1 rounded-full">LIVE</span>
              )}
            </p>
            <p className="text-green-200 text-sm">7% sobre Faturação Hoje</p>
            <p className="text-green-300 text-xs mt-2 italic">
              7% sobre Faturação do Dia ({formatKz(todayRevenue * 0.07)})
            </p>
          </div>
        </div>
