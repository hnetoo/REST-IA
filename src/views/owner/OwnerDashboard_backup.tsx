...
const OwnerDashboard = () => {
  ...
  const fetchMetrics = async () => {
    try {
      ...
      // BUSCAR VENDAS E FATURAÇÃO DE HOJE - USAR QUERY UNIFICADA
      let totalVendas = 0;
      let faturacaoHoje = 0;
      ...
      try {
        ...
        totalVendas = revenueData.total;
        faturacaoHoje = revenueData.total;
        
        console.log('[OWNER HUB] QUERY UNIFICADA - Fonte da Verdade:', {
...
setIsLoading(false);
  ...
  // Subscrição real-time do Supabase - SEM LOOP INFINITO
  useEffect(() => {
    const session = localStorage.getItem('ownerSession');
    if (!session) return;

    log.info('OWNER_DASHBOARD', 'Iniciando subscrição real-time única');
    
    // Forçar atualização inicial dos dados
    fetchMetrics();
    ...
    // Refresh automático a cada 30 segundos (não a cada mudança)
        ...
        <div className="flex items-center gap-3 mb-2 sm:mb-0">
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-white/60 uppercase tracking-wider">Caixa: Equilibrado</span>
            </div>
          </div>
        </div>
