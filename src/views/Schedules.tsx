            ...
            return (
              <div key={shift.id} className="glass-panel p-6 rounded-[2.5rem] border border-white/5 hover:border-primary/30 transition-all group relative overflow-hidden">
                <div className="flex items-center gap-4 mb-6">
                   <div className="w-12 h-12 rounded-2xl flex items-center justify-center text-white shadow-lg text-lg font-black" ref={(el) => { if (el) el.style.backgroundColor = emp?.color || '#334155'; }}>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/90 z-[120] flex items-center justify-center p-6 backdrop-blur-xl animate-in fade-in">
           <div className="glass-panel rounded-[4rem] w-full max-w-md p-12 border border-white/10 shadow-2xl relative">
              <button onClick={() => setIsModalOpen(false)} className="absolute top-10 right-10 text-slate-500 hover:text-white" title="Fechar modal"><X size={32} /></button>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                 <div>
                    ...
                    <select required className="w-full p-5 bg-white/5 border border-white/10 rounded-2xl text-white outline-none focus:border-primary font-bold appearance-none" value={form.employeeId} onChange={e => setForm({...form, employeeId: e.target.value})} aria-label="Selecionar funcionário" title="Selecionar funcionário">
                 ...
                 <div>
                    ...
                    <select required className="w-full p-5 bg-white/5 border border-white/10 rounded-2xl text-white outline-none focus:border-primary font-bold appearance-none" value={form.dayOfWeek} onChange={e => setForm({...form, dayOfWeek: Number(e.target.value)})} aria-label="Selecionar dia da semana" title="Selecionar dia da semana">
                 ...
                 <div className="grid grid-cols-2 gap-4">
                    <div>
                       ...
                       <input required type="time" className="w-full p-5 bg-white/5 border border-white/10 rounded-2xl text-white outline-none focus:border-primary font-mono font-bold" value={form.startTime} onChange={e => setForm({...form, startTime: e.target.value})} placeholder="00:00" title="Horário de entrada" />
                    </div>
                    <div>
                       <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3">Saída</label>
                       <input required type="time" className="w-full p-5 bg-white/5 border border-white/10 rounded-2xl text-white outline-none focus:border-primary font-mono font-bold" value={form.endTime} onChange={e => setForm({...form, endTime: e.target.value})} placeholder="00:00" title="Horário de saída" />
                    </div>
                 </div>
