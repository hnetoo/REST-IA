              ...
              return (
                <div key={emp.id} className="glass-panel p-8 rounded-[3rem] border border-white/5 group hover:border-primary/40 transition-all relative flex flex-col">
                  ...
                  <div className="flex items-center gap-5 mb-8">
                    <div className="w-16 h-16 rounded-[1.8rem] flex items-center justify-center text-white shadow-2xl text-2xl font-black shrink-0 bg-dynamic" style={{'--dynamic-color': emp.color} as React.CSSProperties}>
          ...
          <div className="glass-panel rounded-[4rem] w-full max-w-2xl p-12 border border-white/10 shadow-2xl relative max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSaveEmployee} className="grid grid-cols-2 gap-8">
                ...
                <div>
                  ...
                  <input type="number" min="0" step="1000" className="w-full p-5 bg-white/5 border border-white/10 rounded-2xl text-white outline-none focus:border-primary font-bold" value={empForm.foodAllowance || 0} onChange={e => setEmpForm({...empForm, foodAllowance: Number(e.target.value)})} placeholder="Subsídio alimentação" />
                </div>
                <div>
                  <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3">Subsídio de Transporte (Kz)</label>
                  <input type="number" min="0" step="1000" className="w-full p-5 bg-white/5 border border-white/10 rounded-2xl text-white outline-none focus:border-primary font-bold" value={empForm.transportAllowance || 0} onChange={e => setEmpForm({...empForm, transportAllowance: Number(e.target.value)})} placeholder="Subsídio transporte" />
                </div>
                <div>
                  <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3">Bónus/Prémios (Kz)</label>
                  <input type="number" min="0" step="1000" className="w-full p-5 bg-white/5 border border-white/10 rounded-2xl text-white outline-none focus:border-primary font-bold" value={empForm.bonus || 0} onChange={e => setEmpForm({...empForm, bonus: Number(e.target.value)})} placeholder="Bónus ou prémios" />
                </div>
                <div>
                  <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3">Valor por Hora Extra (Kz)</label>
                  <input type="number" min="0" step="100" className="w-full p-5 bg-white/5 border border-white/10 rounded-2xl text-white outline-none focus:border-primary font-bold" value={empForm.overtimeHourlyRate || 0} onChange={e => setEmpForm({...empForm, overtimeHourlyRate: Number(e.target.value)})} placeholder="Valor hora extra" />
                </div>
