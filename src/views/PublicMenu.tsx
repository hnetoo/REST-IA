...
import { Plus, Package, ShoppingCart, X, Truck, ArrowDown, RefreshCw, AlertTriangle } from 'lucide-react';
...
const PublicMenu = () => {
    ...
    const load = async () => {
      try {
        ...
        console.log('[PublicMenu] Ligação produto-categoria:', productsData?.map(p => ({
          ...
          tem_categoria: categoriesData?.some((c: any) => c.id === p.category_id)
        })));
  ...
  const handleRefresh = useCallback(async () => {
    console.log('[PublicMenu] 🔄 Refresh solicitado via pull-to-refresh');
    setLoading(true);
    ...
    try {
      ...
      if (!productsError && productsData) {
        ...
        console.log('[PublicMenu] ✅ Dados atualizados via pull-to-refresh');
      }
    } catch (error) {
      console.error('[PublicMenu] ❌ Erro no pull-to-refresh:', error);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  return (
    <div 
      ref={containerRef}
      className="h-screen bg-[#0a0f1a] text-white flex flex-col overflow-x-hidden w-full max-w-7xl mx-auto relative"
    >
      ...
      <div className="bg-[#0a0f1a] p-5 flex items-center gap-4 border-b border-gray-800">
        ...
        <div>
          ...
          <p className="text-orange-400 text-xs text-center mt-1 font-bold flex items-center justify-center gap-2">
            <AlertTriangle size={12} />
            Nota: As fotografias são ilustrativas e não representam necessariamente o produto exato.
          </p>
        </div>
      </div>
        ...
        {filteredItems.map((item) => (
          <div key={item.id} className="w-full bg-[#111827] rounded-xl border border-gray-800 flex flex-col overflow-hidden" onClick={() => setSelectedProduct(item)}>
            ...
            <div className="p-3 flex justify-between items-center">
              ...
              <button onClick={(e) => { e.stopPropagation(); addToCart(item); }} className="bg-cyan-500 p-2 rounded-lg text-white flex-shrink-0 ml-2" title="Adicionar ao carrinho">
                <Plus size={20} />
              </button>
            </div>
          </div>
