...
import { getTodayDateString } from '../store/useStore';
import { 
...
const Analytics = () => {
  ...
  const realMetrics = useMemo(() => {
    const today = getTodayDateString(); // 🔑 UNIFICADO: usar função unificada
    ...
    console.log('[ANALYTICS] 📊 Métricas calculadas com dados do Supabase:', {
      totalSalesToday,
      totalOrdersToday,
      ticketMedio,
    ...
    const todayExpenses = expenses.filter(expense => 
      String(expense.createdAt || '').split('T')[0] === today // 🔑 UNIFICADO: toISOString().split('T')[0]
    );
