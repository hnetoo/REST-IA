...
import { 
  Settings as SettingsIcon, Users, Save, Plus, Trash2, KeyRound, 
  Edit2, X, ChefHat,
  Upload, Lock,
  Check, Cpu, Terminal, Binary, RefreshCw,
  UploadCloud, Database, Activity, Trash,
  ShieldCheck, FileBadge, Landmark, Download, Info,
  CloudLightning, Globe, Share2, Server, Link2,
  Utensils, TrendingUp
} from 'lucide-react';
...
const Settings = () => {
  ...
  const tabs = [
    { id: 'GENERAL', label: 'Geral & Identidade', icon: SettingsIcon, desc: 'Nome, logo, moeda' },
    { id: 'STAFF', label: 'Funcionários', icon: Users, desc: 'Equipa e salários' },
    { id: 'OPERATORS', label: 'Controlo de Acesso', icon: Users, desc: 'PINs e permissões' },
    { id: 'FISCAL', label: 'Compliance AGT', icon: ShieldCheck, desc: 'IVA, NIF, regime' },
    { id: 'SUPABASE', label: 'Ecossistema Cloud', icon: CloudLightning, desc: 'Supabase & sync' },
    { id: 'CORE', label: 'Kernel Técnico', icon: Cpu, desc: 'Reset e debug' }
  ];
    ...
    <div className="p-8 h-full overflow-y-auto no-scrollbar bg-background text-slate-200">
      <header className="mb-8">
        <div className="flex items-center gap-2 text-primary mb-2">
          <SettingsIcon size={18} className="animate-pulse" />
          <span className="text-[10px] font-black uppercase tracking-[0.4em]">Configurações do Kernel e Integrações Nuvem</span>
        </div>
        <h2 className="text-4xl font-black text-white italic uppercase tracking-tighter">Sistema & Ecossistema</h2>
      </header>

      <div className="flex gap-3 mb-8 border-b border-white/5 overflow-x-auto no-scrollbar pb-1">
        {tabs.map(tab => (
          <button 
            key={tab.id}
            onClick={() => setActiveTab(tab.id as any)}
            title={tab.label}
            className={`pb-4 px-5 transition-all relative flex flex-col items-start gap-0.5 whitespace-nowrap rounded-t-xl ${activeTab === tab.id ? 'text-primary bg-primary/5' : 'text-slate-500 hover:text-slate-300 hover:bg-white/5'}`}
          >
            <div className="flex items-center gap-2">
              <tab.icon size={14} />
              <span className="font-black uppercase text-[10px] tracking-[0.15em]">{tab.label}</span>
            </div>
            <span className="text-[8px] text-slate-600 font-medium">{tab.desc}</span>
          ...
          <div className="h-[50vh] overflow-y-auto pr-2">
            <form onSubmit={handleSaveSettings} className="max-w-3xl space-y-10 flex flex-col">
             <div className="grid grid-cols-1 gap-8">
                <div>
                   <label htmlFor="restaurantName" className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3">Nome do Restaurante</label>
                   <input 
                     id="restaurantName"
                     type="text" 
                ...
                <div className="flex flex-col lg:flex-row items-start lg:items-center gap-8">
                   <div className="w-32 h-32 lg:w-40 lg:h-40 rounded-3xl bg-white/5 border border-white/10 flex items-center justify-center overflow-hidden shrink-0">
                      {localSettings.appLogoUrl ? <img src={localSettings.appLogoUrl} alt="Logo do Restaurante" className="w-full h-full object-contain p-2" /> : <ChefHat size={48} className="text-slate-800"/>}
                   </div>
          ...
          <div className="space-y-12 max-h-[75vh] overflow-y-auto pr-2 pb-8 scrollbar-thin scrollbar-thumb-orange-500 flex flex-col">
            ...
            <div className="bg-black/60 rounded-[2rem] border border-white/5 p-6 font-mono overflow-hidden">
               ...
               <div className="space-y-2 h-32 overflow-y-auto no-scrollbar">
                  <p className="text-[10px] text-slate-500">[SYSTEM] Kernel inicializado v1.1.2</p>
          ...
          <div className="space-y-12 max-h-[75vh] overflow-y-auto pr-2 pb-8 scrollbar-thin scrollbar-thumb-orange-500 flex flex-col">
             ...
             <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="glass-panel p-8 rounded-[2.5rem] border border-white/5 space-y-6">
                   ...
                   <div className="space-y-4">
                      <div>
                        <label htmlFor="agtCertificate" className="block text-[9px] font-black text-slate-500 uppercase tracking-widest mb-2">N.º do Certificado AGT</label>
                        <input id="agtCertificate" type="text" className="w-full p-4 bg-white/5 border border-white/10 rounded-xl text-white font-mono text-xs" value={localSettings.agtCertificate} onChange={e => setLocalSettings({...localSettings, agtCertificate: e.target.value})} aria-label="Número do Certificado AGT" />
                      </div>
                      <div>
                        <label htmlFor="invoiceSeries" className="block text-[9px] font-black text-slate-500 uppercase tracking-widest mb-2">Série de Faturação Ativa</label>
                        <input id="invoiceSeries" type="text" className="w-full p-4 bg-white/5 border border-white/10 rounded-xl text-white font-mono text-xs uppercase" value={localSettings.invoiceSeries} onChange={e => setLocalSettings({...localSettings, invoiceSeries: e.target.value})} aria-label="Série de Faturação" />
                      </div>
                ...
                <div className="glass-panel p-8 rounded-[2.5rem] border border-white/5 space-y-6">
                   ...
                   <div className="space-y-4">
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label htmlFor="nif" className="block text-[9px] font-black text-slate-500 uppercase tracking-widest mb-2">NIF</label>
                          <input id="nif" type="text" className="w-full p-4 bg-white/5 border border-white/10 rounded-xl text-white font-mono text-xs" value={localSettings.nif} onChange={e => setLocalSettings({...localSettings, nif: e.target.value})} aria-label="NIF" />
                        </div>
                        <div>
                          <label htmlFor="capitalSocial" className="block text-[9px] font-black text-slate-500 uppercase tracking-widest mb-2">Capital Social</label>
                          <input id="capitalSocial" type="text" className="w-full p-4 bg-white/5 border border-white/10 rounded-xl text-white font-mono text-xs" value={localSettings.capitalSocial} onChange={e => setLocalSettings({...localSettings, capitalSocial: e.target.value})} aria-label="Capital Social" />
                        </div>
                      </div>
                      <div>
                ...
                <div key={s.id} className="glass-panel p-6 rounded-[2.5rem] border border-white/5 group hover:border-primary/40 transition-all flex flex-col">
                  <div className="flex justify-between mb-6">
                    ...
                    <div className="flex gap-1">
                      <button onClick={() => handleOpenStaffModal(s)} className="p-3 text-slate-500 hover:text-white" aria-label="Editar funcionário">
                        <Edit2 size={16}/>
                      </button>
                      <button onClick={() => removeStaff(s.id)} className="p-3 text-red-500/30 hover:text-red-500" aria-label="Remover funcionário">
                        <Trash2 size={16}/>
                      </button>
                    </div>
                  </div>
                ...
                {users.map(u => (
                  <div key={u.id} className="glass-panel p-6 rounded-[2.5rem] border border-white/5 group hover:border-primary/40 transition-all flex flex-col">
                     <div className="flex justify-between mb-6">
                        ...
                        <div className="flex gap-1">
                           <button onClick={() => handleOpenUserModal(u)} className="p-3 text-slate-500 hover:text-white" title="Editar utilizador" aria-label="Editar utilizador"><Edit2 size={16}/></button>
                           <button onClick={() => removeUser(u.id)} className="p-3 text-red-500/30 hover:text-red-500" title="Remover utilizador" aria-label="Remover utilizador"><Trash2 size={16}/></button>
                        </div>
                     </div>
             ...
             <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="space-y-6">
                   ...
                   <div className="p-8 bg-black/40 rounded-[2.5rem] border border-white/5 space-y-6">
                      ...
                      <div className="flex items-center gap-4"><Binary className="text-blue-500" size={20}/><span className="text-xs font-mono">Build: REST-IA-OS-v1.1.2</span></div>
                   </div>
                </div>
             </div>
        ...
        <div className="fixed inset-0 bg-black/95 z-[200] flex items-center justify-center p-8 backdrop-blur-xl animate-in zoom-in">
          <div className="glass-panel p-12 rounded-[4rem] w-full max-w-4xl border border-white/10 overflow-y-auto max-h-[calc(100vh-200px)] pr-2 scrollbar-thin scrollbar-thumb-orange-500">
            <div className="flex justify-between items-center mb-10">
              ...
              <button onClick={() => setIsStaffModalOpen(false)} className="text-slate-500 hover:text-white" title="Fechar" aria-label="Fechar">
                <X size={32}/>
              </button>
            </div>
        ...
        <div className="fixed inset-0 bg-black/95 z-[200] flex items-center justify-center p-8 backdrop-blur-xl animate-in zoom-in">
           <div className="glass-panel p-12 rounded-[4rem] w-full max-w-4xl border border-white/10 overflow-y-auto max-h-[calc(100vh-200px)] pr-4 scrollbar-thin scrollbar-thumb-orange-500 scrollbar-track-transparent">
              <div className="flex justify-between items-center mb-10">
                 ...
                 <button onClick={() => setIsUserModalOpen(false)} className="text-slate-500 hover:text-white" title="Fechar" aria-label="Fechar"><X size={32}/></button>
              </div>

              <form onSubmit={handleSaveUser} className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                 <div className="space-y-6">
                    <div>
                       <label htmlFor="user-name" className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3">Nome Completo</label>
                       <input id="user-name" required type="text" className="w-full p-5 bg-white/5 border border-white/10 rounded-2xl text-white outline-none focus:border-primary font-bold" placeholder="Nome" value={userForm.name} onChange={e => setUserForm({...userForm, name: e.target.value})} />
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                       <div>
                          <label htmlFor="user-role" className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3">Função</label>
                          <select id="user-role" title="Selecionar função do utilizador" className="w-full p-5 bg-white/5 border border-white/10 rounded-2xl text-white font-bold outline-none appearance-none" value={userForm.role} onChange={e => setUserForm({...userForm, role: e.target.value as UserRole})}>
                       ...
                       <div>
                          <label htmlFor="user-pin" className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3">PIN (4 Dígitos)</label>
                          <input id="user-pin" required type="password" maxLength={4} className="w-full p-5 bg-white/5 border border-white/10 rounded-2xl text-white text-center font-mono text-xl outline-none" placeholder="PIN" value={userForm.pin} onChange={e => setUserForm({...userForm, pin: e.target.value})} />
                       </div>
                    </div>
