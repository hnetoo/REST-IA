...
const ZoomControl: React.FC<ZoomControlProps> = ({
  ...
  // 🔥 REFS para evitar recriação do event listener
  ...
  const handleKeyPress = useCallback((e: KeyboardEvent) => {
    // 🔥 NÃO INTERFERIR COM INPUTS - Verificar se target é input, textarea ou contentEditable
    ...
    if (isInput) {
      return; // Não interceptar eventos de inputs
    }
    ...
    if (e.ctrlKey || e.metaKey) {
      if (e.key === '+' || e.key === '=') {
        ...
        const newZoom = Math.min(zoomRef.current + step, maxZoom);
        setZoom(newZoom);
      } else if (e.key === '-' || e.key === '_') {
        e.preventDefault();
        const newZoom = Math.max(zoomRef.current - step, minZoom);
        setZoom(newZoom);
      } else if (e.key === '0') {
        e.preventDefault();
        setZoom(defaultZoom);
  ...
  }, [step, minZoom, maxZoom, defaultZoom]);

  // 🔥 CORREÇÃO: Event listener só é registrado uma vez (sem dependências de state)
  useEffect(() => {
    console.log('[ZOOM] 🎯 Registrando listener de teclado (uma vez)');
    document.addEventListener('keydown', handleKeyPress);
    return () => {
      console.log('[ZOOM] 🧹 Removendo listener de teclado');
      document.removeEventListener('keydown', handleKeyPress);
    };
  }, [handleKeyPress]); // handleKeyPress é estável devido ao useCallback
