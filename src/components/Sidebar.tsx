...
import { 
  ...
  Calendar, PartyPopper, BookOpen, ShoppingBag, Boxes
} from 'lucide-react';
...
const Sidebar = () => {
  ...
  const navItems: { to: string; icon: React.ReactNode; label: string; permission?: PermissionKey }[] = [
    ...
    { to: "/inventory", icon: <Package size={20} />, label: "Produtos & Categorias", permission: 'STOCK_MANAGE' },
    { to: "/stock-management", icon: <Boxes size={20} />, label: "Gestão de Stock", permission: 'STOCK_MANAGE' },
    { to: "/compras", icon: <ShoppingCart size={20} />, label: "COMPRAS", permission: 'STOCK_MANAGE' },
    { to: "/reservations", icon: <Calendar size={20} />, label: "RESERVAS", permission: 'POS_SALES' },
    { to: "/events", icon: <PartyPopper size={20} />, label: "EVENTOS", permission: 'FINANCE_VIEW' },
    { to: "/finance", icon: <Banknote size={20} />, label: "Financeiro Legal", permission: 'FINANCE_VIEW' },
    { to: "/sales-control", icon: <ShoppingBag size={20} />, label: "VENDAS", permission: 'FINANCE_VIEW' },
    ...
    { to: "/manual", icon: <BookOpen size={20} />, label: "Manual" },
        ...
        {!isCollapsed && (
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="flex flex-col min-w-0">
                ...
                <span className="text-[8px] font-bold text-primary uppercase tracking-widest opacity-60">REST IA OS v1.1.2</span>
            </div>
          </div>
        )}
      ...
      <nav className="flex-1 px-3 space-y-1 mt-2 overflow-y-auto no-scrollbar">
        {filteredItems.map(item => (
          ...
          >
            ...
            {!isCollapsed && <span className="text-xs font-black uppercase tracking-[0.1em] leading-tight">{item.label}</span>}
          </NavLink>
        ))}
      </nav>
