 ...
 * 🔥 CONGELAMENTO DE DIAS PASSADOS: Orders de dias anteriores não são sincronizadas
 */
import { useEffect } from 'react';
import { orderTransactionService } from '../lib/orderTransactionService';
import { useStore } from '../store/useStore';
import { getSyncablePendingOrders, removePendingSyncOrder } from '../lib/sync/pendingSyncOrders';
...
export function usePendingSyncOrders() {
  ...
  useEffect(() => {
    const syncPending = async () => {
      // 🔥 Usar getSyncablePendingOrders para filtrar dias passados (async)
