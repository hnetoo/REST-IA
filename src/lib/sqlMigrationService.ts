...
export const sqlMigrationService = {
  ...
  async autoMigrate(settings: SystemSettings, localData: any): Promise<{
    ...
    try {
      ...
      if (localData.categories) {
        ...
        for (const category of validCategories) {
          try {
            ...
            // Se não existe no Supabase ou local é mais recente -> criar/atualizar
            if (!existingCategory || localUpdatedAt > remoteUpdatedAt) {
              const updateData: any = { 
                id: category.id,
                name: category.name,
                updated_at: new Date().toISOString()
      ...
      // 1.5 CRIAR CATEGORIA PADRÃO PARA PRODUTOS SEM CATEGORIA
      ...
      try {
        ...
        if (!existingDefault) {
          ...
          console.log('[autoMigrate] ✅ Categoria padrão criada');
        }
      } catch (err) {
        console.warn('[autoMigrate] ⚠️ Erro ao criar categoria padrão:', err);
      }
      if (localData.menu) {
        ...
        for (const product of allDishes) {
          try {
            ...
            if (localUpdatedAt > remoteUpdatedAt) {
              ...
              // 🛡️ VALIDAÇÃO: Garantir category_id válido - usar categoria padrão se necessário
              if (!updateData.category_id || 
                  updateData.category_id === 'undefined' || 
                  updateData.category_id === 'null' ||
                  updateData.category_id === '') {
                // Silent: use default category
                updateData.category_id = DEFAULT_CATEGORY_ID;
              }

              // 🛡️ VALIDAÇÃO: Garantir price é número válido e maior que zero
              if (typeof updateData.price !== 'number' || 
                  isNaN(updateData.price) || 
                  updateData.price < 0) {
                // Silent: skip invalid price
                syncDetails.products.errors++;
                continue;
              }

              // 🛡️ Limpar campos undefined/null/vazios antes de enviar
              ...
              // 🛡️ Garantir campos obrigatórios
              if (!updateData.name || updateData.name.trim() === '') {
                console.error('[autoMigrate] ❌ Produto sem nome, pulando:', productId);
                syncDetails.products.errors++;
                continue;
              ...
              console.log('[autoMigrate] 📦 Enviando produto:', {
                id: productId,
            ...
            if (localUpdatedAt > remoteUpdatedAt) {
              ...
              const { error: updateError } = await supabase
                ...
                .upsert({
                  ...
                  status: order.status === 'closed' ? 'closed' : order.status,
