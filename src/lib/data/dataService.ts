...
class DataServiceBridge {
  ...
  async rehydrateDataFromSupabase(): Promise<void> {
    ...
    try {
      ...
      // 📊 Buscar vendas do dia atual no Supabase (apenas colunas necessárias)
      const { data: vendasHojeData, error: vendasError } = await supabase
        .from('orders')
        .select('id, total_amount, status, created_at, customer_name, table_number, items')
        .eq('status', 'closed')
      ...
      // 💸 Buscar despesas do dia atual no Supabase (apenas colunas necessárias)
      const { data: despesasHojeData, error: despesasError } = await supabase
        .from('expenses')
        .select('id, description, amount_kz, category, status, created_at')
        .gte('created_at', `${hojeWAT}T00:00:00Z`)
        .lte('created_at', `${hojeWAT}T23:59:59Z`);
  ...
  private async getSupabaseMetrics(): Promise<MetricsData> {
    ...
    // 🔑 1. VENDAS DE HOJE - Faturação Hoje (total_amount, status = 'closed', filtro data)
    ...
    if (!vendasHojeError && vendasHojeData) {
      ...
      console.log('[DATA BRIDGE] 📊 Vendas Hoje (closed):', vendasHoje);
    } else {
    ...
    try {
      // Buscar itens das orders com status closed para calcular custo
  ...
  async insertOrder(orderData: any) {
    ...
    if (error) {
      if (error.code === '23505' || error.message?.includes('unique')) {
        console.error('[dataService] ❌ Erro de duplicação ao inserir order:', error);
        return null;
      }
      console.error('[dataService] ❌ Erro ao inserir order:', error);
      return null;
    }
  ...
  async insertExpense(expenseData: any) {
    ...
    if (error) {
      if (error.code === '23505' || error.message?.includes('unique')) {
        console.error('[dataService] ❌ Erro de duplicação ao inserir expense:', error);
        return null;
      }
      console.error('[dataService] ❌ Erro ao inserir expense:', error);
      return null;
    }
