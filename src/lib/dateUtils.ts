...
/**
 * 🔑 Calcula a data_contabil (Dia Operacional) - Lógica IMUTÁVEL:
 * - 05:00 de um dia até 04:59 do dia seguinte = dia atual
 * - Ex: Venda às 03:00 da madrugada → data_contabil = dia anterior
 * - Ex: Venda às 10:00 da manhã → data_contabil = dia atual
 * Timezone: Africa/Luanda (UTC+1)
 */
export const calculateDataContabil = (date: Date = new Date()): string => {
  // Converter para timezone de Luanda
  const zonedDate = toZonedTime(date, APP_TIMEZONE);
  const hour = zonedDate.getHours();
  const day = format(zonedDate, 'yyyy-MM-dd');
  
  // Madrugada (00:00-04:59) → pertence ao dia anterior
  if (hour >= 0 && hour < 5) {
  ...
  // Dia operacional (05:00-23:59) → dia atual
  return day;
};
