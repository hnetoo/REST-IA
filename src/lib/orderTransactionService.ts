...
export const orderTransactionService = {
  ...
  async executeTransaction(
    ...
    // 🔥 VALIDAÇÃO CRÍTICA: Verificar se produtos existem no Supabase
    console.log(`[TRANSACTION:${transactionId}] 🔍 Verificando produtos no Supabase...`);
    ...
    if (productsError) {
      console.error(`[TRANSACTION:${transactionId}] ❌ Erro ao verificar produtos:`, productsError);
      return {
        success: false,
        error: `Erro ao verificar produtos: ${productsError.message}`
      };
    }
    ...
    if (missingProducts.length > 0) {
      console.error(`[TRANSACTION:${transactionId}] ❌ Produtos não encontrados no Supabase:`, missingProducts);
      return {
        success: false,
        error: `Produtos não sincronizados com o Supabase: ${missingProducts.join(', ')}. Sincronize os produtos primeiro.`
      };
    }

    console.log(`[TRANSACTION:${transactionId}] ✅ Todos os ${productIds.length} produtos verificados no Supabase`);
    ...
    try {
      // 🔥 PASSO 1: UPSERT ORDER (evita duplicate key se já existe)
      console.log(`[TRANSACTION:${transactionId}] 📝 Passo 1: Upserting Order...`);
      ...
      if (orderError) {
        console.error(`[TRANSACTION:${transactionId}] ❌ ERRO AO UPSERT ORDER:`, orderError);
        return {
          success: false,
          error: `Falha ao criar/atualizar ordem: ${orderError.message}`
        };
      }

      orderInserted = true;
      console.log(`[TRANSACTION:${transactionId}] ✅ Order upserted com sucesso:`, orderResult?.id);
