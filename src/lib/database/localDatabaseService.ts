...
class LocalDatabaseService {
  ...
  async getLocalMetrics() {
    ...
    const vendasHoje = orders
      ...
      .reduce((acc: number, order: any) => acc + order.total_amount, 0);
    
    const vendasTotais = orders.reduce((acc: number, order: any) => acc + order.total_amount, 0);
    const despesasTotais = expenses.reduce((acc: number, expense: any) => acc + expense.amount_kz, 0);
    
    const folhaSalarial = staff.reduce((acc: number, person: any) => {
    ...
    const historicoExterno = externalHistory.reduce((acc: number, item: any) => acc + item.total_revenue, 0);
